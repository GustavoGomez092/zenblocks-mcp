...
function setGitHubApiKey(apiKey: string): void {
	...
	if (apiKey?.trim()) {
		(
			githubApi.defaults.headers as unknown as Record<string, string>
		...
		delete (githubApi.defaults.headers as unknown as Record<string, string>)
			.Authorization;
 ...
 */
async function getGitHubRateLimit(): Promise<unknown> {
	try {
		...
		if (error instanceof Error) {
			...
			throw new Error("Failed to get rate limit info: Unknown error");
		}
	}
}
